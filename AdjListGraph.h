...
class Graph {
	...
	int numShortestPathCalculatedSourceVerts = 0;
...
vector<int> Graph::shortestPath(int src, bool printOpen, int dest, bool earlyTermination)
{
	...
	if (printOpen)
	{
		...
		for (int i = 0; i < V; ++i)
			matrix[src][i] = dist[i];
		numShortestPathCalculatedSourceVerts++;
		if (numShortestPathCalculatedSourceVerts == V)
		{
			ofstream fout;
			...
			for (int i = 0; i < V; ++i)
			{
				...
				{
					fout << std::fixed << std::setprecision(4) << matrix[i][j] << " ";
				}
				fout << std::endl;
			}
			fout.close();
